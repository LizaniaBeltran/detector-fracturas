...
<style>
    ...
    .main-header { background: linear-gradient(135deg, #1e40af, #3b82f6); padding: 20px; border-radius: 10px; margin-bottom: 20px; }
    .main-header h1 { color: white; font-size: 26px; }
    .main-header p { color: #bfdbfe; font-size: 14px; }
    .card { background: white; border-radius: 10px; padding: 20px; margin: 15px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
    .card h3 { color: #1e40af; font-size: 18px; margin-bottom: 15px; }
    .card h4 { color: #1e40af; font-size: 15px; margin-bottom: 10px; }
    label { color: black !important; }
    .metric-card { background: white; border-radius: 10px; padding: 15px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
    .metric-card h3 { color: #1e40af; font-size: 28px; }
    .metric-card small { color: #666; font-size: 12px; }
    .metric-card.red h3 { color: #dc2626; }
    .metric-card.green h3 { color: #16a34a; }
    .warning-box { background: #fef3c7; border: 2px solid #facc15; border-radius: 8px; padding: 15px; margin: 15px 0; }
    .warning-box p { color: #92400e; font-weight: 600; }
    .legal-box { background: #fef2f2; border: 2px solid #fecaca; border-radius: 8px; padding: 15px; margin: 15px 0; }
    .legal-box h4 { color: #991b1b; }
    .legal-box p { color: #7f1d1d; font-size: 13px; }
    .features-list { background: #eff6ff; border-radius: 10px; padding: 20px; margin: 15px 0; }
    .features-list h4 { color: #1e40af; }
    .features-list li { color: #1e3a8a; font-size: 14px; margin: 8px 0; }
    .sidebar-title { color: white; font-size: 20px; font-weight: bold; }
    ...
    .accuracy-info h4 { color: #1e40af; font-size: 14px; }
        ...
        if uploaded:
            ...
            for r in results:
                ...
                with c3:
                    ...
                    risk_color = "#dc2626" if r['probability'] >= 70 else ("#d97706" if r['probability'] >= 40 else "#16a34a")
                    st.markdown(f"""<br><b style="color:{color};">{r['prediction']}</b>
                    <br><br><b style="font-size:24px;color:{risk_color};">{r['probability']:.1f}%</b>
                    <p style="font-size:12px;color:#666;">Probabilidad de fractura</p>
                    <b style="color:{risk_color};">{risk}</b>
                    <hr style="margin:10px 0;">
                    <p style="font-size:13px;color:#1e40af;text-align:center;"><b>Recomendacion clinica:</b></p>
                    <p style="font-size:12px;color:#333;text-align:center;">{clinical}</p>""", unsafe_allow_html=True)
    ...
    # ============================================
        ...
        if not hist:
            ...
            for sid in sorted(grup.keys(), reverse=True)[:15]:
                ...
                expander_label = f"Estudio {fecha} - {total} imagenes ({fracs} fracturas)"
                if paciente_info:
                    expander_label += f" - {paciente_info}"
                with st.expander(expander_label):
                    for item in items:
                        color = "red" if "fractura" in item['prediction'].lower() and "sin" not in item['prediction'].lower() else "green"
                        st.markdown(f"**{item['prediction']}** **{item['filename']}** - {item['probability']:.1f}%")
